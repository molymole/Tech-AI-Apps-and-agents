...
class AgentFrameworkProductManagementAgent:
    """Wraps Microsoft Agent Framework-based agents to handle MH product management tasks."""
...
```python
class A2AServer:
    """A2A Server wrapper for the MH Product Helper"""
    ...
    def _get_agent_card(self) -> AgentCard:
        """Returns the Agent Card for the MH Product Helper."""
        capabilities = AgentCapabilities(streaming=True)
        
        skill_product_helper = AgentSkill(
            id='product_helper_sk',
            name='MH Product Helper',
            description=(
                'Handles customer inquiries about MH products, including features, pricing, and ranking products based on customer needs.'
            ),
            tags=['product', 'catalog', 'customer-support', 'agent-framework'],
            examples=[
                'Which paint roller is best for smooth surfaces?',
                'Sell me on the benefits of the MH paint sprayer.',
        ...
        agent_card = AgentCard(
            name='MH Product Helper',
            description=(
                'MH Product Helper providing comprehensive product information and recommendations.'
            ),
...
Navigate to `http://127.0.0.1:8001/agent-card/` in your web browser to access the agent card. This will return a JSON response with details about the MH Product Helper agent.

![The MH Product Helper agent card contains details on how to interact with this agent.](../../media/Solution/0301_A2aApp2.png)
